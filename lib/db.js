...
export async function rateMeal(userId, mealName, rating) {
  const { supabase } = await import('./supabase')
  const { data, error } = await supabase
    .from('meal_ratings')
    .upsert({ user_id: userId, meal_name: mealName, rating }, { onConflict: 'user_id,meal_name' })
    .select()
  if (error) throw error
  return data
}
 
export async function getMealRatings(mealNames) {
  const { supabase } = await import('./supabase')
  const { data, error } = await supabase
    .from('meal_rating_averages')
    .select('*')
...
export async function getUserRating(userId, mealName) {
  const { supabase } = await import('./supabase')

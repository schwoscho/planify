...
const TAB_ICONS: Record<string, string> = {
  home: 'ti-home', meals: 'ti-salad', tracker: 'ti-chart-bar',
  grocery: 'ti-shopping-cart', health: 'ti-droplet', assist: 'ti-robot', profile: 'ti-user'
}
const TAB_LABELS: Record<string, string> = {
  home: 'Home', meals: 'Meals', tracker: 'Tracker',
  grocery: 'Grocery', health: 'Health', assist: 'Coach', profile: 'Profile'
}
const AVATARS = ['🥗', '💪', '🔥', '⚡', '🌿', '🏃', '🥑', '👑', '🌟', '🎯']
...
function TDEEInlineCalculator({ goal, currentTdee, onComplete, onCancel }: any) {
  ...
  function calculate() {
    ...
    const tdee = Math.round(bmr * activity); const adj = GADJ[goal || 'maintain'] || 0; const targetCals = tdee + adj
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '1rem' }}>
        {[{ label: 'Age', placeholder: '25', val: age, set: setAge }, { label: 'Weight (kg)', placeholder: '75', val: weight, set: setWeight }, { label: 'Height (cm)', placeholder: '175', val: height, set: setHeight }].map(f => (
          <div key={f.label}>
...
export default function MainApp({ user, profile, onProfileUpdate }: any) {
  ...
  const [mealFilters, setMealFilters] = useState({
    time: null as string | null, diff: null as string | null,
    rtype: [] as string[], cuisine: null as string | null
  })
  const [avoidInput, setAvoidInput] = useState('')
  const [savedToast, setSavedToast] = useState(false)
  const [showSaved, setShowSaved] = useState(false)

  // Food log modal — refs prevent keyboard close
  ...
  function switchTab(newTab: string) { if (newTab === tab) return; setTab(newTab); setTabKey(k => k + 1) }
    ...
    try {
      const res = await fetch('/api/suggest', {
        ...
        body: JSON.stringify({
          profile,
          filters: {
            diet: profile.diet, allergies: profile.allergies, goal: profile.goal,
            budget: profile.budget, recipeType: mealFilters.rtype,
            time: mealFilters.time, difficulty: mealFilters.diff,
            cuisine: mealFilters.cuisine,
          },
          avoid: avoidInput,
        }),
      })
  ...
  async function setWaterGlass(idx: number) {
    ...
    const finalAmount = idx < filled ? Math.min((idx / 5) * waterGoal, waterGoal) : Math.min(((idx + 1) / 5) * waterGoal, waterGoal)
  ...
  function renderHome() {
    return (
      <Dashboard key={tabKey} user={user} profile={{ ...profile, tdee: tgt }} meals={meals} foodLog={foodLog}
        activityLog={activityLog} waterToday={waterToday} waterGoal={waterGoal} waterStreak={waterStreak}
        weightLog={weightLog} activeDay={activeDay}
        ...
        onAddWater={updateWater} onSwitchTab={switchTab} onViewAllMeals={viewAllMeals} />
    )
  }
      ...
      <div key={tabKey} className="anim-fade-slide" style={{ padding: '0 1.25rem 1rem' }}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <div className={`chip pressable ${!showSaved ? 'active-green' : ''}`} onClick={() => setShowSaved(false)}>
            <i className="ti ti-calendar-week" style={{ fontSize: '13px', marginRight: '4px' }} />This week
          </div>
          ...
          <div>
            {!savedRecipes.length ? (
              ...
              <div key={recipe.id} className="card anim-fade-slide" style={{ marginBottom: '10px', position: 'relative' as const }}>
                <button className="pressable" onClick={async () => { await handleSaveRecipe(recipe) }}
                  style={{ position: 'absolute' as const, top: '12px', right: '12px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-red)', fontSize: '18px', lineHeight: 1 }}>
                  <i className="ti ti-heart-filled" style={{ fontSize: '18px' }} />
                </button>
                ...
                {recipe.macros && (
                  <div style={{ display: 'flex', gap: '6px', marginBottom: '10px' }}>
                    {[{ val: recipe.macros.calories, label: 'kcal', color: 'var(--color-amber)' }, { val: recipe.macros.protein + 'g', label: 'protein', color: 'var(--color-primary)' }, { val: recipe.macros.carbs + 'g', label: 'carbs', color: 'var(--color-blue)' }, { val: recipe.macros.fat + 'g', label: 'fat', color: 'var(--color-purple)' }].map(m => (
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="pressable" onClick={async () => { await selectMeal({ ...recipe, desc: recipe.description }); setShowSaved(false) }}
                    ...
                    Add to {DAYS[activeDay]} <i className="ti ti-arrow-right" style={{ fontSize: '12px' }} />
                  </button>
          ...
          <div>
            {profile?.goal && (
              <div className="profile-banner">
                <i className="ti ti-check" style={{ fontSize: '16px', color: 'var(--color-primary)' }} />
            ...
            <div className="overflow-x-auto flex gap-2" style={{ marginBottom: '12px' }}>
              {[
                { id: 'all', label: 'All' },
                { id: 'quick', label: 'Quick', icon: 'ti-bolt', type: 'time' },
                { id: 'medium', label: 'Medium', icon: 'ti-clock', type: 'time' },
                { id: 'weekend', label: 'Weekend', icon: 'ti-chef-hat', type: 'time' },
                { id: 'easy', label: 'Easy', icon: 'ti-mood-smile', type: 'diff' },
                { id: 'advanced', label: 'Advanced', icon: 'ti-certificate', type: 'diff' },
              ].map(f => {
                ...
                return (
                  <div key={f.id} className={`chip pressable ${isActive ? (f.type === 'diff' ? 'active-amber' : 'active-green') : ''}`}
                    onClick={() => {
                      ...
                      else setMealFilters(m => ({ ...m, diff: m.diff === f.id ? null : f.id }))
                    }}>
                    {f.icon && <i className={`ti ${f.icon}`} style={{ fontSize: '12px', marginRight: '3px' }} />}
                    {f.label}
                  </div>
                )
              })}
            ...
            {todayMeal ? (
              <div className="card anim-scale-in" style={{ borderColor: 'var(--color-primary-border)', background: 'var(--color-primary-pale)', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="pressable" onClick={() => { setMealModalOpen(true); setMealSuggestions([]) }}
                    ...
                    <i className="ti ti-refresh" style={{ fontSize: '13px', marginRight: '4px' }} />Change meal
                  </button>
                  <button className="pressable" onClick={() => addToGrocery(activeDay)}
                    ...
                    <i className="ti ti-shopping-cart" style={{ fontSize: '13px', marginRight: '4px' }} />Add to grocery
                  </button>
                </div>
              </div>
            ) : (
              <div className="pressable" onClick={() => { setMealModalOpen(true); setMealSuggestions([]) }}
                ...
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--color-primary-pale)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className="ti ti-plus" style={{ fontSize: '18px', color: 'var(--color-primary)' }} />
                </div>
            ...
            {plannedCount < 7 && (
              <div style={{ background: 'var(--color-primary-pale)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <i className="ti ti-clipboard-list" style={{ fontSize: '16px', color: 'var(--color-primary)' }} />
      ...
      <div key={tabKey} className="anim-fade-slide" style={{ padding: '0 1.25rem 1rem' }}>
        ...
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '10px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--color-text-muted)', marginTop: '2px' }}>{profile?.goal ? GL[profile.goal] : 'Set a goal'}{profile?.tdee ? ' · TDEE ✓' : ''}</div>
          </div>
        </div>
        <div className="card" style={{ marginBottom: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="ti ti-fork" style={{ fontSize: '15px', color: 'var(--color-amber)' }} />Food log
            </div>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="ti ti-run" style={{ fontSize: '15px', color: 'var(--color-red)' }} />Activity
            </div>
    ...
    return (
      <div key={tabKey} className="anim-fade-slide" style={{ padding: '0 1.25rem 1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '19px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <i className="ti ti-shopping-cart" style={{ fontSize: '20px', color: 'var(--color-primary)' }} />Grocery list
          </div>
          {grocery.some((g: any) => g.checked) && (
            <button className="pressable" onClick={async () => { await saveGroceryItems(user.id, weekStart, grocery.filter((g: any) => !g.checked)); await loadGrocery() }}
              style={{ fontSize: '12px', color: 'var(--color-text-muted)', background: 'none', border: `1px solid var(--color-border)`, borderRadius: '6px', padding: '4px 10px', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <i className="ti ti-trash" style={{ fontSize: '13px' }} />Clear done
            </button>
          )}
        </div>
        {!grocery.length ? (
      ...
      <div key={tabKey} className="anim-fade-slide" style={{ padding: '0 1.25rem 1rem' }}>
        ...
        <div className="anim-scale-in" style={{ background: waterStreak === 0 ? 'linear-gradient(135deg,#6b7280,#4b5563)' : waterStreak >= 7 ? 'linear-gradient(135deg,#E74C3C,#E67E22)' : 'linear-gradient(135deg,#E67E22,#D4833A)', borderRadius: '16px', padding: '1.25rem', marginBottom: '12px', textAlign: 'center' as const, color: '#fff', boxShadow: waterStreak > 0 ? '0 4px 20px rgba(230,126,34,0.3)' : 'none' }}>
          {waterStreak >= 7
            ...
            : <i className={`ti ti-flame ${waterStreak >= 1 ? 'anim-pulse' : ''}`} style={{ fontSize: '40px', display: 'block', marginBottom: '4px', color: '#fff' }} />
          }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '8px', marginBottom: '10px' }}>
          {Array.from({ length: 5 }, (_, i) => (
            <div key={i} className={goalReached ? '' : 'pressable'} onClick={() => !goalReached && setWaterGlass(i)}
              style={{ aspectRatio: '1', borderRadius: '10px', border: `1.5px solid ${i < filledGlasses ? 'var(--color-blue)' : 'var(--color-blue-border)'}`, background: i < filledGlasses ? 'var(--color-blue-pale)' : 'var(--color-surface)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '3px', transition: 'all 0.2s', opacity: goalReached ? 0.8 : 1, cursor: goalReached ? 'default' : 'pointer' }}>
        ...
        {goalReached && (
          <div className="anim-scale-in" style={{ background: 'var(--color-primary-pale)', border: `1px solid var(--color-primary-border)`, borderRadius: '10px', padding: '10px 14px', marginBottom: '12px', textAlign: 'center' as const, fontSize: '13px', color: 'var(--color-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            <i className="ti ti-circle-check" style={{ fontSize: '18px' }} />Goal reached! Locked until tomorrow.
          </div>
        )}
        ...
        {latestWeight && (
          <div className="card" style={{ marginBottom: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              {weightChange !== null && <span style={{ fontSize: '13px', fontWeight: '500', padding: '4px 10px', borderRadius: '20px', background: weightChange > 0 ? 'var(--color-primary-pale)' : 'var(--color-red-pale)', color: weightChange > 0 ? 'var(--color-primary)' : 'var(--color-red)', display: 'flex', alignItems: 'center', gap: '3px' }}>
                <i className={`ti ${weightChange > 0 ? 'ti-arrow-up' : 'ti-arrow-down'}`} style={{ fontSize: '12px' }} />
        ...
        <div className="card" style={{ marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button className="pressable" onClick={logWeight}
              style={{ padding: '10px 16px', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <div className="anim-fade-slide" style={{ alignSelf: 'flex-start', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
              ...
              <div style={{ background: 'var(--color-surface)', border: `1px solid var(--color-border)`, borderRadius: '16px', borderBottomLeftRadius: '4px', padding: '14px', maxWidth: '80%' }}>
                ...
                <div style={{ fontSize: '13px', lineHeight: '1.6' }}>
                  Hi! I'm your personal nutrition coach. Ask me anything — meal ideas, macro advice, or anything nutrition-related!
    ...
    return (
      <div key={tabKey} className="anim-fade-slide" style={{ padding: '0 1.25rem 1rem' }}>
        <div style={{ textAlign: 'center' as const, marginBottom: '1.5rem' }}>
          ...
          {profile?.tdee && (
            <div style={{ marginTop: '6px', display: 'inline-flex', alignItems: 'center', gap: '5px', background: 'var(--color-primary-pale)', color: 'var(--color-primary)', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '500' }}>
              <i className="ti ti-target" style={{ fontSize: '13px' }} />{profile.tdee} kcal daily target
            </div>
          )}
        </div>
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <div className="card" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', padding: '11px 14px' }}>
            <span style={{ fontSize: '13px', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="ti ti-mail" style={{ fontSize: '14px' }} />Email
            </span>
          ...
          <div className="card pressable" onClick={() => setEditModalOpen('tdee')}
            style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', padding: '11px 14px', cursor: 'pointer' }}>
            <span style={{ fontSize: '13px', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="ti ti-calculator" style={{ fontSize: '14px' }} />Calorie target
            </span>
            <span style={{ fontSize: '13px', fontWeight: '500', color: 'var(--color-primary)', display: 'flex', alignItems: 'center', gap: '3px' }}>
              {profile?.tdee ? `${profile.tdee} kcal` : 'Not calculated'} <i className="ti ti-arrow-right" style={{ fontSize: '13px' }} />
            </span>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', padding: '12px 14px' }}>
            <div>
              <div style={{ fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <i className="ti ti-moon" style={{ fontSize: '14px' }} />Dark theme
              </div>
          ...
          <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', padding: '12px 14px' }}>
            <div>
              <div style={{ fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <i className="ti ti-bell" style={{ fontSize: '14px' }} />Notifications
              </div>
          ...
          <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px' }}>
            <div style={{ fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <i className="ti ti-info-circle" style={{ fontSize: '14px' }} />App version
            </div>
  ...
  return (
    <div style={{ maxWidth: '420px', margin: '0 auto', minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--color-bg)', transition: 'background var(--transition-slow)' }}>
      ...
      <div style={{ padding: '1rem 1.25rem .75rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid var(--color-border-subtle)` }}>
        <img src="/images/logo.png" alt="Planify" style={{ height: '26px', width: 'auto', objectFit: 'contain' }} />
      ...
      {/* Bottom nav — Tabler icons */}
      <nav className="bottom-nav">
        {TABS.map(t => (
          <button key={t} className={`nav-btn pressable ${tab === t ? 'active' : ''}`} onClick={() => switchTab(t)}>
            {t === 'assist'
              ? <img src="/images/coach.png" alt="Coach" style={{ width: '22px', height: '22px', borderRadius: '50%', objectFit: 'cover' }} />
              : <i className={`ti ${TAB_ICONS[t]} nav-icon`} style={{ fontSize: '22px' }} />
            }
      ...
      <Modal open={mealModalOpen} onClose={() => setMealModalOpen(false)} title="Suggest a meal" subtitle={`for ${DAYS[activeDay]}${profile?.goal ? ' · ' + GL[profile.goal] : ''}`}>
          ...
          <div key={i} className="anim-fade-slide" style={{ background: 'var(--color-primary-pale)', border: `1.5px solid var(--color-primary-border)`, borderRadius: '12px', padding: '12px', marginBottom: '8px', position: 'relative' as const }}>
            <div className="pressable" onClick={() => selectMeal(meal)}>
              ...
              <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' as const }}>
                ...
                {meal.cuisine && <span className="tag tag-green">{CUISINES.find(c => c.value === meal.cuisine)?.flag} {meal.cuisine}</span>}
              ...
              <div style={{ fontSize: '11px', color: 'var(--color-primary)', marginTop: '6px', opacity: .7, display: 'flex', alignItems: 'center', gap: '3px' }}>
                <i className="ti ti-hand-click" style={{ fontSize: '12px' }} />Tap to add to {DAYS[activeDay]}
              </div>
            </div>
            <button className="pressable" onClick={async (e) => { e.stopPropagation(); await handleSaveRecipe(meal) }}
              style={{ position: 'absolute' as const, top: '10px', right: '10px', background: 'none', border: 'none', cursor: 'pointer', lineHeight: 1, color: 'var(--color-text-muted)' }}
              title="Save recipe">
        ...
        {mealSuggestions.length > 0 && (
          <div style={{ fontSize: '12px', color: 'var(--color-primary-light)', textAlign: 'center' as const, marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
            <i className="ti ti-hand-click" style={{ fontSize: '13px' }} />Tap to add · <i className="ti ti-heart" style={{ fontSize: '13px' }} />to save
          </div>
        )}

        <button className="pressable" onClick={getSuggestions} disabled={suggestLoading}
          style={{ width: '100%', padding: '13px', background: suggestLoading ? 'var(--color-border)' : 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: suggestLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          {suggestLoading
      ...
      <Modal open={logModalOpen} onClose={() => setLogModalOpen(false)} title="Log food" subtitle="Add something you ate or drank today.">
        ...
        {logResult && (
          <div className="anim-scale-in" style={{ background: 'var(--color-primary-pale)', border: `1px solid var(--color-primary-border)`, borderRadius: '8px', padding: '10px 12px', fontSize: '13px', color: 'var(--color-primary)', marginBottom: '.75rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <button className="pressable" onClick={logFood} disabled={logLoading}
          style={{ width: '100%', padding: '13px', background: logLoading ? 'var(--color-border)' : 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: logLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          {logLoading
            ? <><i className="ti ti-loader-2 ti-spin" style={{ fontSize: '16px' }} />Looking up...</>
            : <><i className="ti ti-search" style={{ fontSize: '16px' }} />Look up & add</>
          }
        </button>
      ...
      <Modal open={actModalOpen} onClose={() => setActModalOpen(false)} title="Log activity" subtitle="Add exercise to track calories burned.">
        ...
        <div style={{ position: 'relative' as const, marginBottom: '1rem' }}>
          <select value={actType || ''} onChange={e => {
            ...
            style={{ width: '100%', padding: '12px 40px 12px 14px', borderRadius: '12px', border: `1.5px solid ${actType ? 'var(--color-primary-light)' : 'var(--color-border)'}`, background: actType ? 'var(--color-primary-pale)' : 'var(--color-surface)', fontSize: '14px', color: actType ? 'var(--color-primary)' : 'var(--color-text-muted)', fontFamily: 'inherit', outline: 'none', appearance: 'none' as const, cursor: 'pointer', transition: 'all 0.18s' }}>
        ...
        {actType && (
          <div className="anim-scale-in" style={{ background: 'var(--color-primary-pale)', border: `1px solid var(--color-primary-border)`, borderRadius: '10px', padding: '10px 14px', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <i className={`ti ${ACTIVITIES.find(a => a.value === actType)?.icon || 'ti-activity'}`} style={{ fontSize: '24px', color: 'var(--color-primary)' }} />
            <div>
        ...
        <input ref={actDurRef} type="number" placeholder="Duration in minutes (e.g. 45)"
          onChange={e => {
            ...
            if (actType && dur) { const act = ACTIVITIES.find(a => a.value === actType); setActBurnPreview(Math.round((act?.burn || 6) * dur)) }
            else setActBurnPreview(null)
          }}
          className="input" style={{ marginBottom: '.75rem' }} />

        {actBurnPreview !== null && (
          <div className="anim-scale-in" style={{ background: 'var(--color-red-pale)', border: `1px solid var(--color-red-border)`, borderRadius: '8px', padding: '8px 12px', fontSize: '13px', color: 'var(--color-red)', marginBottom: '.75rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <button className="pressable" onClick={logActivity} disabled={!actType}
          style={{ width: '100%', padding: '13px', background: !actType ? 'var(--color-border)' : 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: !actType ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
      ...
      <Modal open={editModalOpen === 'avatar'} onClose={() => setEditModalOpen(null)} title="Choose your avatar">
        ...
        <button className="pressable" onClick={async () => { try { const updated = await saveProfile(user.id, { ...profile, avatar }); onProfileUpdate(updated); setEditModalOpen(null) } catch(e){} }}
          style={{ width: '100%', padding: '13px', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
      ...
      <Modal open={editModalOpen === 'tdee'} onClose={() => setEditModalOpen(null)} title="Recalculate calorie target" subtitle="Update your daily targets based on current stats.">
        <TDEEInlineCalculator
          goal={profile?.goal} currentTdee={profile?.tdee}
          onComplete={async (targets: any) => {
            try { const updated = await saveProfile(user.id, { ...profile, tdee: targets.tdee, protein_target: targets.protein, carbs_target: targets.carbs, fat_target: targets.fat }); onProfileUpdate(updated); setEditModalOpen(null) } catch(e) { console.error(e) }
          }}

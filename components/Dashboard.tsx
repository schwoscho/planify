...
export default function Dashboard({ user, profile, meals, foodLog, activityLog, waterToday, waterGoal, waterStreak, weightLog, onAddMeal, onLogFood, onLogActivity, onAddWater, onSwitchTab, onViewAllMeals, activeDay, onGoToProfile, avatarEmoji }: DashboardProps) {
  ...
  return (
    <div className="anim-fade-slide" style={{padding:'1rem 1.25rem 1rem'}}>
      ...
      {/* Greeting row — avatar inline with name */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.25rem'}}>
        <div>
          <div style={{fontSize:'13px',color:'var(--color-text-muted)'}}>{greeting},</div>
          <div style={{fontFamily:'var(--font-display)',fontSize:'24px',fontWeight:'600',color:'var(--color-text)',letterSpacing:'-0.5px',lineHeight:1.1}}>{displayName}</div>
          {profile?.goal&&(
        ...
        <div className="pressable" onClick={onGoToProfile}
          style={{width:'48px',height:'48px',borderRadius:'50%',background:'linear-gradient(135deg,var(--color-primary),var(--color-primary-light))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'26px',cursor:'pointer',boxShadow:'0 2px 10px rgba(45,106,79,0.25)',flexShrink:0}}>
          {avatarEmoji}
        </div>
      </div>
      ...
      <div className="card anim-fade-slide" style={{marginBottom:'10px',padding:'1.25rem'}}>
        <div style={{display:'flex',alignItems:'center',gap:'1.25rem'}}>
          ...
          <div style={{flex:1}}>
            ...
            {[
              {label:'Eaten',val:totalIn,color:'var(--color-amber)',icon:'🍴'},
              {label:'Burned',val:totalBurned,color:'var(--color-red)',icon:'🔥'},
              {label:'Target',val:tgt,color:'var(--color-primary)',icon:'🎯'},
            ].map(item=>(
              <div key={item.label} style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'5px'}}>
                <span style={{fontSize:'12px',color:'var(--color-text-muted)'}}>{item.icon} {item.label}</span>
        ...
        {todayMeal?.macros&&(
          <div style={{display:'flex',gap:'6px',marginTop:'10px',paddingTop:'10px',borderTop:'0.5px solid var(--color-border)'}}>
            {[{label:'Protein',val:todayMeal.macros.protein+'g',color:'var(--color-primary)'},{label:'Carbs',val:todayMeal.macros.carbs+'g',color:'var(--color-blue)'},{label:'Fat',val:todayMeal.macros.fat+'g',color:'var(--color-purple)'}].map(m=>(
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px',marginBottom:'10px'}}>
        <div className="card pressable" onClick={()=>onSwitchTab('health')}
          ...
          <div style={{display:'flex',gap:'4px',width:'100%'}}>
            {[250,500].map(ml=>(
              <button key={ml} onClick={e=>{e.stopPropagation();if(!goalReached)onAddWater(ml)}}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
          <div className="pressable" onClick={()=>onSwitchTab('health')}
            ...
            <span className={waterStreak>=1?'anim-pulse':''} style={{fontSize:'28px',display:'block',marginBottom:'4px'}}>🔥</span>

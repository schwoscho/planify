...
const GL: Record<string, string> = {
  bulk: 'Bulking', cut: 'Cutting', maintain: 'Balanced',
  energy: 'Energy boost', gut: 'Gut health'
}
...
export default function Dashboard({ user, profile, meals, foodLog, activityLog, waterToday, waterGoal, waterStreak, weightLog, onAddMeal, onLogFood, onLogActivity, onAddWater, onSwitchTab, onViewAllMeals, activeDay }: DashboardProps) {
  ...
  return (
    <div className="anim-fade-slide" style={{ padding: '0 1.25rem 1rem' }}>
      ...
      <div style={{ marginBottom: '1.25rem' }}>
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '24px', fontWeight: '600', color: 'var(--color-text)', letterSpacing: '-0.5px' }}>{displayName}</div>
        {profile?.goal && (
          <div style={{ fontSize: '12px', color: 'var(--color-primary)', marginTop: '2px', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <i className="ti ti-target" style={{ fontSize: '13px' }} />
            {GL[profile.goal]} · €{profile.budget}/week
          </div>
        )}
      </div>
      ...
      <div className="card anim-fade-slide" style={{ marginBottom: '10px', padding: '1.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {[
              { label: 'Eaten', val: totalIn, color: 'var(--color-amber)', icon: 'ti-fork' },
              { label: 'Burned', val: totalBurned, color: 'var(--color-red)', icon: 'ti-flame' },
              { label: 'Target', val: tgt, color: 'var(--color-primary)', icon: 'ti-target' },
            ].map(item => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
        <button className="pressable" onClick={onLogFood}
          style={{ padding: '14px 12px', borderRadius: '12px', border: `1.5px solid var(--color-border)`, background: 'var(--color-surface)', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' as const }}>
      ...
      <div className="card anim-fade-slide" style={{ marginBottom: '10px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <div style={{ fontSize: '13px', fontWeight: '500', color: 'var(--color-text)', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <i className="ti ti-salad" style={{ fontSize: '15px', color: 'var(--color-primary)' }} />
            Today's meal
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
        <div className="card pressable" onClick={() => onSwitchTab('health')}
          ...
          <div style={{ fontSize: '11px', fontWeight: '600', color: 'var(--color-text-muted)', textTransform: 'uppercase' as const, letterSpacing: '.07em', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
            <i className="ti ti-droplet" style={{ fontSize: '13px', color: 'var(--color-cyan)' }} /> Water
          </div>
          ...
          <div style={{ display: 'flex', gap: '3px', marginBottom: '8px', width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <div className="pressable" onClick={() => onSwitchTab('health')}
            ...
            <i className={`ti ti-flame ${waterStreak >= 1 ? 'anim-pulse' : ''}`} style={{ fontSize: '28px', color: waterStreak > 0 ? '#fff' : 'var(--color-text-muted)', display: 'block', marginBottom: '4px' }} />
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: '600', color: waterStreak > 0 ? '#fff' : 'var(--color-text-muted)', lineHeight: 1 }}>{waterStreak}</div>
            <div style={{ fontSize: '10px', color: waterStreak > 0 ? 'rgba(255,255,255,0.7)' : 'var(--color-text-muted)', marginTop: '2px', textTransform: 'uppercase' as const, letterSpacing: '.06em' }}>day streak</div>
          </div>
          {latestWeight && (
            <div className="card pressable" onClick={() => onSwitchTab('health')} style={{ cursor: 'pointer', textAlign: 'center' as const }}>
              <div style={{ fontSize: '11px', fontWeight: '600', color: 'var(--color-text-muted)', textTransform: 'uppercase' as const, letterSpacing: '.07em', marginBottom: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                <i className="ti ti-scale" style={{ fontSize: '13px' }} /> Weight
              </div>
      ...
      <div className="card anim-fade-slide">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          <div style={{ fontSize: '13px', fontWeight: '500', color: 'var(--color-text)', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <i className="ti ti-calendar-week" style={{ fontSize: '15px', color: 'var(--color-primary)' }} />
            Weekly meal plan
          </div>
          ...
          {DAYS.map((d, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: meals[i] ? 'var(--color-primary)' : i === activeDay ? 'var(--color-primary-pale)' : 'var(--color-bg)', border: `1.5px solid ${meals[i] ? 'var(--color-primary)' : i === activeDay ? 'var(--color-primary-light)' : 'var(--color-border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                {meals[i] ? <i className="ti ti-check" style={{ fontSize: '14px', color: '#fff' }} /> : <span style={{ fontSize: '11px', color: i === activeDay ? 'var(--color-primary)' : 'var(--color-text-muted)' }}>{d[0]}</span>}
              </div>
            </div>
          ))}
        </div>
